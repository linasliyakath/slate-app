        ...
        <div className="w-full pt-4 md:pt-10">
            {isEditing ? (
                <div className="space-y-6 md:space-y-8 animate-in fade-in duration-300">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-3 pt-2 group cursor-pointer" onClick={() => setEditCompleted(!editCompleted)}>
                            ...
                            <span className="text-base md:text-lg font-light text-zinc-300 group-hover:text-white transition-colors">Mark as Completed</span>
                        </div>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-3 md:gap-4 pt-6 md:pt-8 border-t border-zinc-900">
                        <button
                            onClick={handleUpdate}
                            className="bg-white hover:bg-zinc-200 text-black px-6 md:px-8 py-3 rounded-none transition-all active:scale-95 font-bold tracking-widest text-xs uppercase"
                        >
                            Save Changes
                        </button>
                        <button
                ...
                <div className="animate-in fade-in duration-500">
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-8 md:mb-12 border-b border-zinc-900 pb-6 md:pb-10">
                        <div className="flex flex-col gap-3 md:gap-4 flex-1">
                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 md:gap-4">
                                <h1 className={`text-3xl md:text-4xl lg:text-5xl font-light tracking-tighter text-white ${task.completed ? "line-through text-zinc-600" : ""}`}>{task.title}</h1>
                        ...
                        <div className="flex gap-2 md:gap-3 w-full sm:w-auto">
                            <button
                                ...
                                className="text-zinc-400 border border-zinc-800 hover:bg-zinc-900 hover:text-white hover:border-zinc-700 px-4 md:px-6 py-2 text-[10px] transition-all uppercase tracking-[0.2em] font-bold flex-1 sm:flex-none"
                            >
                                Edit
                            </button>
                            <button
                                onClick={handleDelete}
                                className="text-rose-900 border border-rose-900/20 hover:border-rose-900 hover:bg-rose-950/30 hover:text-rose-500 px-4 md:px-6 py-2 text-[10px] transition-all uppercase tracking-[0.2em] font-bold flex-1 sm:flex-none"
                            >
                                Delete
                            </button>
                        </div>
                    ...
                    <div className="prose max-w-none">
                        <h3 className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.3em] mb-4 md:mb-6">Description</h3>
                        <div className="bg-zinc-900/20 border border-zinc-900/50 p-4 md:p-8 min-h-[10rem] md:min-h-[12rem]">
                            <p className="whitespace-pre-wrap text-base md:text-lg text-zinc-300 leading-relaxed font-light">{task.description || <span className="text-zinc-700 italic">No description provided.</span>}</p>
                        </div>
                    </div>
                </div>
            )}
